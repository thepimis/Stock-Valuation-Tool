  ...
  async fetch(request) {
    ...
    try {
      ...
      if (url.searchParams.has('search')) {
        ...
        return jsonResponse(await searchYahooEquities(query));
      }
      ...
      if (url.searchParams.has('quote') || url.searchParams.has('priceTicker')) {
        ...
        return jsonResponse(await fetchYahooQuote(ticker));
      }
...
async function fetchYahooJson(path, params = {}) {
  let lastError = null;
...
async function searchYahooEquities(query) {
  ...
  const allowedExchanges = new Set(['NYQ', 'NMS', 'NGM', 'NCM', 'ASE']);
  const exchangeLabels = { NYQ: 'NYSE', NMS: 'NASDAQ', NGM: 'NASDAQ', NCM: 'NASDAQ', ASE: 'NYSE American' };
  const results = (Array.isArray(body.quotes) ? body.quotes : [])
    .filter(row => String(row.quoteType || '').toUpperCase() === 'EQUITY')
    .filter(row => allowedExchanges.has(String(row.exchange || row.exchDisp || '').toUpperCase()))
    .map(row => {
      ...
      return {
        ...
        exchangeCode,
        exchange: exchangeLabels[exchangeCode] || row.exchDisp || exchangeCode,
        quoteType: 'EQUITY'
      };
    })
    .filter(row => row.symbol);
  return { ok: true, version: VERSION, query, results };
}
...
async function fetchYahooQuote(ticker) {
  ...
  const allowed = new Set(['NYQ', 'NMS', 'NGM', 'NCM', 'ASE']);
  if (quoteType && quoteType !== 'EQUITY') throw createError(400, `${ticker} is not an equity.`);
  if (exchangeCode && !allowed.has(exchangeCode)) throw createError(400, `${ticker} is not a supported US-listed equity.`);
  return {
    ok: true,
    version: VERSION,
    ticker,
...
async function calculateFcfMarginHistory(ticker) {
  ...
  const [quarterlyRevenueResult, annualCashFlowResult, annualRevenueResult, yahooResult] = await Promise.all([
    ...
    fetchYahooTtmCashFlow(ticker).then(value => ({ ok: true, value })).catch(error => ({
      ok: false,
...
async function fetchYahooTtmCashFlow(ticker) {
  const types = [
  ...
  for (const host of YAHOO_HOSTS) {
    ...
    try {
      ...
      const operatingFact = extractLatestYahooFact(result, 'trailingOperatingCashFlow');
      const capexFact = extractLatestYahooFact(result, 'trailingCapitalExpenditure');
      const directFcfFact = extractLatestYahooFact(result, 'trailingFreeCashFlow');
      ...
      // Yahoo's visible cash-flow table defines FCF as OCF minus CapEx. Use that
      // exact calculation; expose Yahoo's direct FCF fact for validation.
...
function extractLatestYahooFact(result, type) {
  const candidates = [];
